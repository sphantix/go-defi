...
import (
	...
	pancakev3pool "github.com/sphantix/go-defi/bindings/pancake/v3pool"
...
func (c *Client) GetLiquidityFromPool(pool common.Address) (*big.Int, error) {
	caller, err := pancakev3pool.NewPancakev3poolCaller(pool, c.bc)
	if err != nil {
		return nil, errors.Wrap(err, "new pancake v3 pool caller")
	}
...
func (c *Client) GetFactory(pool common.Address) (common.Address, error) {
	caller, err := pancakev3pool.NewPancakev3poolCaller(pool, c.bc)
	if err != nil {
		return common.Address{}, errors.Wrap(err, "new pancake v3 pool caller")
	}
